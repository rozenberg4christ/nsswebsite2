...
{
$usr=$_REQUEST['userid'];
echo'<script language=javascript>window.location.href="vol_settings1.php?uid='.base64_encode($usr).'"</script>';
}
if(isset($_REQUEST['inauguration']))
{
$update=mysql_query("UPDATE volunteer SET status='Y' where log_status='N'");
echo'<script language=javascript>window.location.href="vol_settings.php"</script>';
}
$qqqq=mysql_query("select * from tbl_settings") or die(mysql_error());
$rrrr=mysql_fetch_array($qqqq);
$StartRow = 0;
$StartRow1 = 0;
$sql1="select * from tbl_login";
...
?>
  ...
  <tr>
    <td width="12%">Search User ID : </td>
    <td width="10%"><input style="border: 1px solid #cecece;font-size:14px;background: #fefefe url(css/layout/site/forms/input_text2.gif) repeat-x;width: 94px; height:20px;margin: 0px 8px 0 0;" name="userid"  id="userid"/></td>
    <td width="66%" align="left"><input type="submit" name="search" id="search" value="Search" onclick="return serval();" /></td>
    <td width="12%"><span class="button" id="testclick1"><strong>Inauguration</strong></span>
<script type="text/javascript">
	T$('testclick1').onclick = function(){TINY.box.show('popup.php',1,800,415,1)}
</script></td>
  </tr>
</table>
															...
															<td style="width: 56px;">
																<div class="actions">
																	<ul>
																		...
																		<li><a class="edit" href="edit_vol.php?id=<?php echo base64_encode($res['id']); ?>">2</a></li>
						...
						?>
												...
												<input type="button" name="add" id="add" value="Add User" onclick="window.location.href='addlogin.php';" />
