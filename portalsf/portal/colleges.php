...
$StartRow = 0;
$StartRow1 = 0;
$sql1="select * from tbl_colleges where col_status='Y'";
...
/*$sql3="select * from tbl_colleges where col_status='N'";
...
{ 
	$deleteSQL = sprintf("update %s set %s WHERE col_id='%s'",$thetable,$values,$theid);
...
if(isset($_REQUEST['typea']))
{
	...
	while($i<=$rowsperpage)
	{
		...
		if($_REQUEST['selecta'.$i]!="")
		{
			...
			$sql=mysql_query("select * from tbl_colleges where col_id='".$erecord."'")or die(mysql_error());
			$fetch=mysql_fetch_array($sql);
			$values="col_status='".$status."'";
			editrecord("tbl_colleges",$values,$erecord); 
		}
		$i++;
	} 
					...
					<td>													
													<table cellpadding="0" cellspacing="0" width="100%">
														<tbody><tr>
															<th width="50">No.</th>
															<th width="43">Code</th>
															<th width="440">College Name</th>
															<th width="68">Date</th>
															<th width="71">Status</th>
															<th width="60" style="width: 56px;">Actions</th>
														</tr>
														<?php
